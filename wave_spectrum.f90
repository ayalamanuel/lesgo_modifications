...
use param, only : L_x, L_y, pi, nx, ny, alpha_spec, wp_spec, z_i, theta_main, theta_spread
use sim_param, only: K_wave, D_spread, S_kx_ky, phi, omega_wave, eta_hat_o
implicit none

complex(kind=8) :: ii
integer :: jx, jy
real(rprec) :: grav, delta_kx, delta_ky, sigma_spec, &
               L_x_wave, L_y_wave
...
theta_spread = theta_spread*pi/180_rprec
...
do jy = 1, ny
   do jx = 1, nx
      ...
      if (abs(Theta_wave(jx,jy)-theta_main) <= theta_spread/2.0_rprec) then
          D_spread(jx,jy) = (2.0_rprec/theta_spread)*cos(pi*(Theta_wave(jx,jy) - &
                            theta_main)/theta_spread)**2
      else
          D_spread(jx,jy) = 0.0
      end if
...
use param, only : write_endian, checkpoint_wave_spectrum
...
! distribution needs to be  available for the calculation of deta_dt in 
! wallstress.f90
...
inquire (file='wave_spectrum.out', exist=wave_spectrum_file_flag)
