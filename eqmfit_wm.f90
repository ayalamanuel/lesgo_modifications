...
use sim_param, only : eqmxz, eqmyz, ur_mag_eqm, u_delta_m, u, v
use grid_m
...
if (smooth_eqm) then
        eqmxz = -(Re_fit(:,:)/Re_delta(:,:))**2 * ur_eqm(:,:)*u_delta_m(:,:)
        eqmyz = -(Re_fit(:,:)/Re_delta(:,:))**2 * vr_eqm(:,:)*u_delta_m(:,:)
        ...
        A = ((Re_fit(:,:)/Re_delta(:,:))**(6.0_rprec) + (2.5_rprec* &
            log((delta_fit)/zo))**(-6.0_rprec))**(0.33333_rprec)
        eqmxz = - A(:,:) * ur_eqm(:,:)*u_delta_m(:,:)
        eqmyz = - A(:,:) * vr_eqm(:,:)*u_delta_m(:,:)
