...
use read_wave, only : read_wave_file
...
! Initialize the wave spectrum variables or read the wave files 
if (lbc_mom == 4 .or. lbc_mom == 6) then
if (wave_type == 1) then
    call spectrum_calc()
endif

if (wave_type == 3 .and. coord == 0) then
    call read_wave_file()
end if
end if
...
! Iniialize integral wall model xiang
if (lbc_mom == 3) then
