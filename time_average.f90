...
type tavg_t
    real(rprec), dimension(:,:,:), allocatable :: u, v, w, u_w, v_w, w_uv, p, pres_real
    real(rprec), dimension(:,:,:), allocatable :: u2, v2, w2, uv, uw, vw, uw_uv
    real(rprec), dimension(:,:,:), allocatable :: txx, tyy, tzz, txy, txz, tyz
    real(rprec), dimension(:,:), allocatable :: eqmxz, wpmxz, eqmyz, wpmyz
    real(rprec), dimension(:,:,:), allocatable :: vortx, vorty, vortz, cs_opt2,fx, fy, fz
...
!  variable quantity
!
use param, only : ubc_mom, lbc_mom, coord, nproc, dz, nx, ny, jt_total
use sgs_param, only : Cs_opt2
use sim_param, only : u, v, w, p
use sim_param, only : txx, txy, tyy, txz, tyz, tzz, dvdx, dudy, dwdy, dvdz, dudz, dwdx
...
vortz(1:nx,1:ny,lbz:nz) = dvdx(1:nx,1:ny,lbz:nz) - dudy(1:nx,1:ny,lbz:nz)
...
!*******************************************************************************
use grid_m
use param, only : write_endian, lbz, path, coord, nproc, mombal_debug
use string_util
#ifdef PPMPI
use mpi_defs, only : mpi_sync_real_array,MPI_SYNC_DOWNUP
...
character(64) :: fname_f, fname_rs, fname_cs, fname_vort, fname_mosd
...
real(rprec), allocatable, dimension(:,:,:) :: up2, vp2, wp2, upvp, upwp, vpwp, upwp_uv
...
write(13,rec=7) this%uw_uv(:nx,:ny,1:nz)
close(13)
...
write(13,rec=7) upwp_uv(:nx,:ny,1:nz)
close(13)
