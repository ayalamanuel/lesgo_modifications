...
use sim_param ,only: Cx_wave, Cy_wave, s_wpmxz, eqmxz
implicit none
...
! For waveplane and mosdplane data output, all processors call inst_write
! to avoid MPI deadlock, but only coord=0 actually writes data to file.
! Since all these variables are available at nz=1 for each processor, 
! we do not want to have redundancy of output data.
if(waveplane_calc) then
    if (jt_total >= waveplane_nstart .and. jt_total <= waveplane_nend .and. &
        ...
        call inst_write(6)  ! All processors call this
    end if
end if
if(mosdplane_calc) then
    if (jt_total >= mosdplane_nstart .and. jt_total <= mosdplane_nend .and. &
        ...
        call inst_write(7)  ! All processors call this
    end if
end if
...
use param, only : dx, dy, rest_print, opt1
use param, only : write_endian,jt_total
use grid_m
use sim_param, only : u, v, w, p, dpdx
use sim_param, only : dwdy, dwdx, dvdx, dudy, dudx, dwdz, txx, txz, dudz
use sim_param, only : txz, tyz
use sim_param, only : eqmxz, eqmyz, s_wpmxz, s_wpmyz
use sim_param, only : eta, detadx, detady, detadt, Cx_wave, Cy_wave,  &
                      S_kx_ky, K_wave, D_spread, eta_hat_o, phi, u_orb, w_orb
use functions, only : interp_to_w_grid
use derivatives

use stat_defs, only : xplane, yplane
#ifdef PPMPI
use stat_defs, only : zplane, point, spacetime
...
! Vorticity
real(rprec), dimension (:,:,:), allocatable :: vortx, vorty, vortz, vortz_uv
...
if (lbc_mom == 4 .or. lbc_mom == 6) then
...
use stat_defs, only : point, xplane, yplane, zplane, spacetime
implicit none
